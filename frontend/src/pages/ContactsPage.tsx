...
export const ContactsPage: React.FC = () => {
  ...
  const onRemove = async (contactId: string): Promise<void> => {
    try {
  ...
  const openShare = async (): Promise<void> => {
    try {
      const data = (await api.get("/contacts/share")).data as ShareData;
      setShareData(data);
      setShareOpen(true);
    } catch (err: any) {
      toast({
        variant: "error",
        title: "Could not load share info",
        description: err.message,
      });
    }
  };
  ...
  const copyHandle = async (): Promise<void> => {
    ...
    try {
      await navigator.clipboard.writeText(
        shareData.share_text || shareData.handle,
      );
      setCopied(true);
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          <Button
            type="submit"
            size="sm"
            disabled={!handle.trim() || addContact.isPending}
          >
          ...
          {shareData && (
            <div className="space-y-3 mt-2">
              <div className="relative rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] p-3">
                <p className="text-sm pr-10 break-all">
                  {shareData.share_text || shareData.handle}
                </p>
                <Button
                  size="icon-sm"
