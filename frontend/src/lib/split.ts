interface Member {
  id: string;
}

interface SplitMember {
  user_id: string;
  value?: number;
  share_cents?: number;
}

interface SplitData {
  type?: string;
  members: SplitMember[];
}

interface ShareResult {
  user_id: string;
  share_cents: number;
}

interface CustomShareEdit {
  split?: SplitMember[];
  userId: string;
  cents: number;
  amountCents: number;
}

interface BudgetStatus {
  state: "exact" | "under" | "over";
  remainderCents: number;
}

interface SplitPayload {
  split_type: string;
  custom_split: SplitMember[];
}

export function defaultSplitData(
  type: string,
  members: Member[],
  amountCents: number,
): SplitMember[] {
  const ids = members.map((m) => m.id);
  const n = ids.length || 1;
  if (type === "equal") return ids.map((id) => ({ user_id: id, value: 1 }));
  if (type === "custom")
    return distributeEqualSplit(amountCents, ids).map((s) => ({
      user_id: s.user_id,
      value: s.share_cents,
    }));
  if (type === "percentage")
    return ids.map((id) => ({
      user_id: id,
      value: parseFloat((100 / n).toFixed(2)),
    }));
  if (type === "shares") return ids.map((id) => ({ user_id: id, value: 1 }));
  return [];
}

export function computeShare(
  split: SplitData | null | undefined,
  amountCents: number,
  userId: string,
): number {
  if (!split || !split.members) return 0;
  const member = split.members.find((m) => m.user_id === userId);
  if (!member) return 0;
  const type = split.type || "equal";
  if (type === "custom") return (member.value ?? 0) as number;
  if (type === "percentage")
    return Math.round(((member.value ?? 0) / 100) * amountCents);
  if (type === "shares") {
    const total = split.members.reduce((s, m) => s + (m.value ?? 0), 0);
    return total > 0
      ? Math.round(((member.value ?? 0) / total) * amountCents)
      : 0;
  }
  const shares = distributeEqualSplit(
    amountCents,
    split.members.map((m) => m.user_id),
  );
  return shares.find((s) => s.user_id === userId)?.share_cents ?? 0;
}

export function distributeEqualSplit(
  amountCents: number,
  memberIds: string[] = [],
): ShareResult[] {
  const n = memberIds.length;
  if (n === 0) return [];
  const base = Math.floor(amountCents / n);
  const rem = amountCents - base * n;
  return memberIds.map((id, i) => ({
    user_id: id,
    share_cents: base + (i < rem ? 1 : 0),
  }));
}

export function normalizeCustomSplit(split: SplitMember[] = []): SplitMember[] {
  return split
    .filter((item) => item?.user_id)
    .map((item) => ({
      user_id: item.user_id,
      share_cents: Number.isFinite(item.share_cents)
        ? Math.max(0, Math.trunc(item.share_cents ?? 0))
        : 0,
    }));
}

export function sumSplitShares(split: SplitMember[] = []): number {
  return split.reduce((sum, item) => sum + (item.share_cents ?? 0), 0);
}

export function applyCustomShareEdit({
  split = [],
  userId,
  cents,
  amountCents,
}: CustomShareEdit): SplitMember[] {
  const parsed = Number.isFinite(cents) ? Math.trunc(cents) : 0;
  const clampedCents = Math.max(0, Math.min(amountCents, parsed));
  const existing = split.find((s) => s.user_id === userId);

  const next = existing
    ? split.map((s) =>
        s.user_id === userId ? { ...s, share_cents: clampedCents } : s,
      )
    : [...split, { user_id: userId, share_cents: clampedCents }];

  if (next.length === 2) {
    return next.map((s) =>
      s.user_id === userId
        ? { ...s, share_cents: clampedCents }
        : { ...s, share_cents: amountCents - clampedCents },
    );
  }

  return next;
}

export function isSplitReady({
  splitType,
  split,
  amountCents,
}: {
  splitType: string;
  split: SplitMember[];
  amountCents: number;
}): boolean {
  if (!split.length) return false;
  if (splitType !== "custom") return true;
  return sumSplitShares(split) === amountCents;
}

export function getCustomSplitBudgetStatus({
  splitType,
  split = [],
  amountCents = 0,
}: {
  splitType: string;
  split?: SplitMember[];
  amountCents?: number;
}): BudgetStatus {
  if (splitType !== "custom") return { state: "exact", remainderCents: 0 };
  const remainderCents = (amountCents ?? 0) - sumSplitShares(split);
  if (remainderCents === 0) return { state: "exact", remainderCents: 0 };
  if (remainderCents > 0) return { state: "under", remainderCents };
  return { state: "over", remainderCents: Math.abs(remainderCents) };
}

function haveSameMembers(
  memberIds: string[] = [],
  split: SplitMember[] = [],
): boolean {
  if (!memberIds.length) return false;
  const allIds = [...memberIds].sort().join(",");
  const splitIds = split
    .map((s) => s.user_id)
    .sort()
    .join(",");
  return allIds === splitIds;
}

export function resolveSplitPayload({
  splitType,
  split,
  allMemberIds = [],
  forceCustom = false,
}: {
  splitType: string;
  split: SplitMember[];
  allMemberIds?: string[];
  forceCustom?: boolean;
}): SplitPayload {
  const normalized = normalizeCustomSplit(split);

  if (forceCustom || splitType === "custom") {
    return { split_type: "custom", custom_split: normalized };
  }

  if (splitType === "equal") {
    if (haveSameMembers(allMemberIds, normalized)) {
      return { split_type: "equal", custom_split: [] };
    }
    return { split_type: "custom", custom_split: normalized };
  }

  return { split_type: "custom", custom_split: normalized };
}
