import { NavLink, Outlet, Link, NavLinkRenderProps } from "react-router-dom";
import {
  Users,
  LayoutGrid,
  Shield,
...
export function AppShell(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    setUserHandle((first as any)?.handle);
    invalidateGlobal(qc);
  }, [allUsers, qc]);
  ...
  const handleSwitch = (user: User): void => {
    setPickerOpen(false);
  ...
  const activeUser: User | undefined =
    (me as any) ||
    allUsers.find((u: any) => u.handle === currentHandle) ||
    allUsers[0];
        ...
        <div className="border-t border-[var(--color-border)] p-3">
          <div className="relative" ref={pickerRef}>
            ...
            >
              ...
              <ChevronsUpDown
                className="h-3.5 w-3.5 text-[var(--color-muted-foreground)] shrink-0"
                aria-hidden
              />
            </button>
