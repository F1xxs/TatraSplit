...
function evaluateExpression(raw: string): number | null {
  ...
  const collapsed: (number | string)[] = [tokens[0]];
...
function toCalculatorDisplay(expr: string): string {
  return expr.replace(/\*/g, "×").replace(/\//g, "÷").replace(/\./g, ",");
}

function fromCalculatorDisplay(expr: string): string {
...
export const MoneyInput: React.FC<MoneyInputProps> = ({
  ...
  const appendExpression = (token: string): void => {
    setCalcError("");
    setExpression((prev) => {
  ...
  const computeExpression = (): number | null => {
  ...
  const textSize =
    size === "xl" ? "text-5xl" : size === "lg" ? "text-3xl" : "text-2xl";

  return (
    <>
      <div
        className={cn(
          "flex items-baseline justify-center gap-2 font-semibold tabular-nums",
          className,
        )}
      >
        <span
